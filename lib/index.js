...
function resolveLayoutBundle() {
  ...
  try {
    const packagePath = require.resolve(`${LAYOUT_PACKAGE}/package.json`);
    const pkg = JSON.parse(readFileSync(packagePath, 'utf8'));
    const client = pkg?.exports?.['./client'];
    const relative = typeof client === 'string'
      ? client
      : client && typeof client === 'object' && typeof client.default === 'string'
        ? client.default
        : null;
    if (!relative) return null;
    const file = join(dirname(packagePath), relative);
...
export function apply(ctx, config = {}, internals = {}) {
  ...
  // Desktop advanced mode removes the official ui-layout row, while phone
  // access runs the desktop client in compatibility mode. Reinsert the
  // official layout bundle through the proxy instead of showing an
  // "unsupported" overlay. It is idempotent when the host still provides it.
  const layoutBundle = isDesktop ? resolveLayoutBundle() : null;
  if (isDesktop && !layoutBundle) {
    logger.warn('dsh-pocket: layout bundle unavailable; advanced-mode phone access may fail | 布局包不可用，高级模式手机访问可能失败');
  }
