...
/** Boot 图脚本形态：window.__DSH_BOOT__ = <json>。 */
const BOOT_SCRIPT_RE = /(<script[^>]*>)window\.__DSH_BOOT__\s*=\s*(.*?)(<\/script>)/gs;

/**
 * 在 Desktop advanced 模式下把官方 layout 行回插进手机页面 boot 图。
 * 桌面窗口本身不经过此代理，因此不会改变桌面布局。
 */
export function restoreLayoutEntry(html, layout) {
  ...
  return html.replace(BOOT_SCRIPT_RE, (match, openTag, raw, closeTag) => {
    let graph;
    try { graph = JSON.parse(raw); } catch { return match; }
...
function normalizeLayoutBundle(bundle, log) {
  ...
  try {
    ...
    log?.(`dsh-pocket: layout bundle unreadable, restore disabled: ${err?.message ?? err}`);
    return null;
  }
 ...
 * @param {{id:string, file:string, map?:string}|null} [opts.layoutBundle] Official layout bundle to restore for Desktop advanced-mode phones.
 * @returns {Promise<{server:import('node:http').Server, close:()=>Promise<void>}>}
 */
export function createPocketProxy({ port = 3081, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, layoutBundle = null, auth = null, rateLimit = null, heartbeat = {}, lanAccessEnabled = () => true } = {}) {
