 ...
 */
export const RANDOM_UUID_POLYFILL = `<script data-dsh-pocket-polyfill="1">/* issue #58：局域网访问时 location.hostname 是局域网 IP，DSH 客户端据此判定
isLoopback=false → settings mirror 降级 memory 模式 → 模型设置页
"settings are unavailable in this browser"。location.hostname 是 WebIDL
unforgeable 属性无法直接覆盖；改用全局 let location 词法绑定遮蔽 +
Proxy 转发（其他属性/方法转发真实 location，只把 hostname 伪装成 127.0.0.1）。
DSH 的 API 基址走 globalThis.location.origin（不受影响）。*/
let __dshPocketLoc = new Proxy(window.location, { get: function (t, p) { if (p === 'hostname') return '127.0.0.1'; if (p === 'reload' || p === 'assign' || p === 'replace') return t[p].bind(t); return t[p]; }, set: function (t, p, v) { t[p] = v; return true; } });
let location = __dshPocketLoc;
