...
| 桌面版高级模式手机白屏 `Failed to load plugins` / `waiting for service: layout` | v1.4.8 起自动修复：桌面版 `dsh-desktop.mode: advanced` 会禁用官方布局插件，手机又被本插件强制 compatibility，两端都没有 layout 服务 → boot 失败。代理现在会把官方 `@deepseek-ai/dsh-client-ui-layout` 回插进手机页面的插件图并自托管其 bundle（**桌面端零改动**），手机恢复官方布局 + 完整移动端适配。v1.4.9 修复窄屏下会话标题、正文、composer 控件和状态行的横向溢出；v1.4.10 让 composer 的工具区与模型/发送区分行，并将其一级、二级菜单收回手机视口；v1.4.11 重新编排模型/上下文控制并让对话状态参数完整换行；v1.4.12 让额外注入的圆形控制稳定排在模型之后，避免回到“添加”之前。更新到 ≥1.4.12 并重启即可 |
